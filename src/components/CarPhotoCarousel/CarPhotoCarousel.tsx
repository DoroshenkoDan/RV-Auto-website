...
export function CarPhotoCarousel({
  ...
  const photos = gallery
    .map((item) => item.image)
    .filter((image): image is Media => typeof image === "object" && Boolean(image?.url))
  ...
  const controlsClassName = cn(
    ...
    "disabled:opacity-0 [&_svg]:size-5 cursor-pointer",
  ...
  return (
    <Carousel className={cn("group/carousel relative", fill && "h-full", className)}>
      ...
      {photos.length > 1 && (
        <>
          <CarouselPrevious variant="ghost" className={cn("left-3", controlsClassName)} />
          <CarouselNext variant="ghost" className={cn("right-3", controlsClassName)} />
        </>
      )}
    </Carousel>
  )
